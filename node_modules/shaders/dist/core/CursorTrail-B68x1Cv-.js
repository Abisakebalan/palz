import { Bt as ZERO, Et as mixColorStopsRuntime, Ht as call, J as std, Jt as vec4, Kt as floatE, Tn as d, X as createGuardedCompute, Z as createStateBuffer, ln as colorMixing_exports, n as tgpu } from "./coords-BkIjixe_.js";
...
var decayLayout = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(tgpu.bindGroupLayout({
	trailBuf: {
		storage: d.arrayOf(d.vec4f, CELL_COUNT),
		access: "mutable"
	},
	params: { uniform: /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(d.struct({
		fadeRate: d.f32,
		ageRate: d.f32,
		shrink: d.f32
	}), "uniform") }
}), "decayLayout");
var stampLayout = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(tgpu.bindGroupLayout({
	trailBuf: {
		storage: d.arrayOf(d.vec4f, CELL_COUNT),
		access: "mutable"
	},
...
var outputLayout = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(tgpu.bindGroupLayout({
	...
	trailTex: { storageTexture: d.textureStorage2d(STATE_FORMAT, "write-only") }
}), "outputLayout");
const decayKernel = tgpu.fn([d.u32, d.u32])(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((cx, cy) => {
	"use gpu";
	const p = decayLayout.$.params;
	const idx = __tsover_add(__tsover_mul(cy, d.u32(GRID_SIZE)), cx);
	...
	const age = std.min(__tsover_add(cell.z, p.ageRate), 1);
	decayLayout.$.trailBuf[idx] = d.vec4f(intensity, size, age, 0);
}), {
	v: 1,
	name: void 0,
	ast: {
...
const stampKernel = tgpu.fn([d.u32, d.u32])(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((cx, cy) => {
	"use gpu";
	const p = stampLayout.$.params;
	const cellX = __tsover_div(__tsover_add(d.f32(cx), .5), d.f32(GRID_SIZE));
	const cellY = __tsover_div(__tsover_add(d.f32(cy), .5), d.f32(GRID_SIZE));
	const ddx = std.select(__tsover_sub(cellX, p.cursorX), __tsover_mul(__tsover_sub(cellX, p.cursorX), p.aspect), p.aspect >= 1);
	const ddy = std.select(__tsover_div(__tsover_sub(cellY, p.cursorY), p.aspect), __tsover_sub(cellY, p.cursorY), p.aspect >= 1);
	...
	if (distSq < __tsover_mul(influenceRadius, influenceRadius)) {
		const idx = __tsover_add(__tsover_mul(cy, d.u32(GRID_SIZE)), cx);
		const influence = std.exp(__tsover_mul(__tsover_div(distSq, __tsover_mul(p.radius, p.radius)), -1));
		const cell = stampLayout.$.trailBuf[idx];
		const newIntensity = std.min(__tsover_add(cell.x, __tsover_mul(influence, p.amount)), 1);
		stampLayout.$.trailBuf[idx] = d.vec4f(newIntensity, 1, 0, 0);
	}
}), {
	v: 1,
	name: void 0,
...
const outputKernel = tgpu.fn([d.u32, d.u32])(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((cx, cy) => {
	...
	std.textureStore(outputLayout.$.trailTex, d.vec2u(cx, cy), cell);
}), {
	v: 1,
	name: void 0,
	ast: {
...
const cursorTrailTapUV = tgpu.fn([
	d.vec2f,
	d.f32,
	d.f32
], d.vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, ox, oy) => {
	"use gpu";
	return d.vec2f(__tsover_add(uv.x, ox), __tsover_add(uv.y, oy));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var TrailField = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(d.struct({
	age: d.f32,
	mask: d.f32
}), "TrailField");
const cursorTrailField = tgpu.fn([
	d.vec4f,
	d.vec4f,
	d.vec4f,
	d.vec4f,
...
const componentDefinition = {
	...
	compute: (params) => {
		...
		const root = gpu?.root;
		if (!root) return null;
		const trailBuf = createStateBuffer(root, d.vec4f, CELL_COUNT);
		...
		const decayParams = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(root.createUniform(d.struct({
			fadeRate: d.f32,
			ageRate: d.f32,
			shrink: d.f32
		})), "decayParams");
		const stampParams = /* @__PURE__ */ (globalThis.__TYPEGPU_AUTONAME__ ?? ((a) => a))(root.createUniform(d.struct({
			cursorX: d.f32,
			cursorY: d.f32,
			radius: d.f32,
			amount: d.f32,
		...
		const decay = createGuardedCompute(root, /* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((cx, cy) => {
			...
			ast: {
				...
				"externalNames": ["decayKernel"]
			},
			externals: () => ({ decayKernel })
		}) && $.f)({}), {
			size,
			bindGroup: decayBg
		});
		const stamp = createGuardedCompute(root, /* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((cx, cy) => {
			...
			ast: {
				...
				"externalNames": ["stampKernel"]
			},
			externals: () => ({ stampKernel })
		}) && $.f)({}), {
			size,
			bindGroup: stampBg
		});
			...
			getComputeNodes: (frameParams) => {
				...
				if (speed > .001) {
					...
					for (let s = 0; s < numSteps; s++) {
						...
						const cursorX = oldX + dx * t;
						const cursorY = oldY + dy * t;
						nodes.push(() => stampParams.write({
							cursorX,
							cursorY,
							radius,
