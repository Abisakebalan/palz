...
const Shader = ({ children, disableTelemetry = false, colorSpace = "p3-linear", toneMapping = "linear", isPreview = false, onReady, onUnavailable, style = {}, className = "",...rest }) => {
	const containerRef = useRef(null);
	const canvasRef = useRef(null);
	const reactId = useId();
	const rootId = useMemo(() => "shader-root-" + reactId.replace(/[^a-zA-Z0-9_]/g, ""), [reactId]);
	const rendererRef = useRef(null);
	if (rendererRef.current === null) rendererRef.current = shaderRendererGPU();
	const onReadyRef = useRef(onReady);
	onReadyRef.current = onReady;
	const onUnavailableRef = useRef(onUnavailable);
	onUnavailableRef.current = onUnavailable;
	const gpuUnavailableRef = useRef(null);
	useEffect(() => {
	...
	const startTelemetryWhenReady = () => {
		...
		const checkRendering = () => {
			...
			if (rendererRef.current.getPerformanceStats().fps > 0) {
				telemetryCollectorRef.current = startTelemetry(rendererRef.current, "3.0.445", disableTelemetry, isPreview);
	...
	const initializeRenderer = async () => {
		...
		if (!canvas || isInitializedRef.current || isInitializingRef.current || gpuUnavailableRef.current) return;
