import { JSX } from 'solid-js';
import { BlendMode, PropDriver, TransformConfig, BoundingBoxConfig } from '../../core';
import { ComponentProps } from '../../core/shaders/ReactionDiffusion';
export type { ComponentProps };
/**
 * Base props that all shader components have
 */
interface BaseShaderProps {
    children?: JSX.Element;
    blendMode?: BlendMode;
    opacity?: number;
    visible?: boolean;
    id?: string;
    maskSource?: string;
    maskType?: string;
    renderOrder?: number;
    transform?: Partial<TransformConfig>;
    boundingBox?: Partial<BoundingBoxConfig>;
}
/**
 * Component-specific props that merge base props with shader-specific props
 * Note: ComponentProps are made optional since they have defaults from the shader definition
 */
type ShaderComponentProps = BaseShaderProps & Partial<Omit<ComponentProps, 'contrast' | 'threshold' | 'relief' | 'lightAngle' | 'childInfluence' | 'childContrast' | 'childThreshold'>> & {
    contrast?: ComponentProps['contrast'] | PropDriver;
    threshold?: ComponentProps['threshold'] | PropDriver;
    relief?: ComponentProps['relief'] | PropDriver;
    lightAngle?: ComponentProps['lightAngle'] | PropDriver;
    childInfluence?: ComponentProps['childInfluence'] | PropDriver;
    childContrast?: ComponentProps['childContrast'] | PropDriver;
    childThreshold?: ComponentProps['childThreshold'] | PropDriver;
};
/**
 * The main Solid wrapper component for Shader shader nodes
 */
export default function ShaderComponent(props: ShaderComponentProps): JSX.Element;
//# sourceMappingURL=ReactionDiffusion.d.ts.map